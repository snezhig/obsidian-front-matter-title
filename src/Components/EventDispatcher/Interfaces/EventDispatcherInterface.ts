...
export default interface EventDispatcherInterface<E> {
    addListener<T extends keyof E>(listener: { name: T, cb: Callback<E[T]>, sort: number, once: boolean }): ListenerRef<T>;
...
export class EventDispatcher<E> implements EventDispatcherInterface<E> {
    private readonly events: Map<keyof E, {cb: Callback<E[keyof E]>, sort: number, once: boolean, ref: ListenerRef<keyof E>}[]> = new Map();

    addListener<T extends keyof E>({name, cb, sort = 100, once = false}: { name: T; cb: Callback<E[T]>; sort?: number; once?: boolean }): ListenerRef<T> {
