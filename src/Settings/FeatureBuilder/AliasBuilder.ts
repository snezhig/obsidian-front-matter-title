...
export default class AliasBuilder extends AbstractBuilder<Feature.Alias> {
    ...
    build({id, name, desc, settings}: BuildParams<Feature.Alias>): void {
        this.id = id;
        this.desc = desc;
        this.setting = new Setting(this.context.containerEl)
            .setName(name)
            .setDesc(desc);
    ...
    private buildDropdown(value: string): void {
        this.setting.addDropdown(c => this.dropdown = c.addOptions({
            ensure: 'Ensure',
            adjust: 'Adjust',
            replace: 'Replace'
        }).setValue(value ? value : 'ensure').onChange(this.onChange.bind(this)));
    }
    ...
    private onChange(): void {
        this.context.getDispatcher().dispatch(
            ...
            new Event({id: this.id, value: {enabled: this.toggle.getValue(), strategy: this.dropdown.getValue()}})
        );
        this.actualizeDesc();
    }
