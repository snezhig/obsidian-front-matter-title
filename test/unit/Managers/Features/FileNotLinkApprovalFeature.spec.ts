...
test('Should add listener and be enabled', async () => {
    mockDispatcher.addListener.mockImplementationOnce((n, c: CallbackInterface<AppEvents["note:link:change:approve"]>) => cb = c);
...
describe('Test callback', () => {
    const event = new Event<AppEvents["note:link:change:approve"]>({changes: [], path: '', approve: Promise.resolve(true)});
