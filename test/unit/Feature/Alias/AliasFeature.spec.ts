...
test("Should add listener, set validator, strategy and refresh", () => {
    ...
    expect(mockDispatcher.addListener).toHaveBeenCalledWith({ name: "metadata:cache:changed", cb: expect.anything() });
    expect(mockValidatorFactory).toHaveBeenCalledTimes(1);
    expect(mockValidatorFactory).toHaveBeenCalledWith(validatorType);
    expect(mockStrategyFactory).toHaveBeenCalledTimes(1);
    expect(mockStrategyFactory).toHaveBeenCalledWith(strategyType);
