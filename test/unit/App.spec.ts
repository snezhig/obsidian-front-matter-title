...
import { DeprecatedFeature } from "@src/enum";
...
Container.rebind(SI.feature).toDynamicValue(() => mock<FeatureInterface<DeprecatedFeature>>());
...
describe("Test App", () => {
    ...
    describe('Test "settings.changed event"', () => {
        ...
        test("Should change templates and dispatch new event", () => {
            ...
            expect(spy.dispatch).toHaveBeenCalledWith(
                "templates:changed",
                new Event({
                    old: [],
                    new: ["actual_title"],
